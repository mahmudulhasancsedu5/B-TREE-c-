...
void split_parent(treeNode* par, treeNode *child1, treeNode* child2, char val[])
{
    char* mid;
    int i, j, flag = 0;
    ...
    sort(dup->val[0], dup->val[0]+dup->vals+1);
    ...
    else if(par->parent->vals < N-1)
    {
        ...
        for(i = 0; i < N-1; i++)
        {
            ...
            if(strcmp(par->parent->val[i],mid)>0)
            {
                for(j=N-2; j>=i; j--)
                {
...
void split_leaf(treeNode* n, char val[])
{
    char* mid;
    int i, j;
    treeNode *n1 = NULL, *n2 = NULL, *n3 = NULL;
    ...
    else if(n->parent->vals < N-1)
    {
        ...
        for(i = 0; i < N-1; i++)
        {
            ...
            if(strcmp(n->parent->val[i],mid)>0)
            {
                for(j=N-2; j>=i; j--)
                {
...
void insertion(char val[])
{
    ...
    else
    {
        ...
        if(temp->vals < N-1)
        {
            ...
            sort(temp->val[0], temp->val[0]+temp->vals);
        }
        else
            split_leaf(temp, val);
...
int main()
{
    ...
    printf("1 for inserting data\n");

    printf("3 for printing the b+ tree\n");
    printf("4 for exit\n\n");
